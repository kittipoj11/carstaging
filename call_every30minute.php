...
foreach ($rs as $row) :
  ...
  // endif;
  // $_SESSION['push'] = pushMsg($arrayHeader, $arrayPostData);

  $booking->update_time($row['id'], date("H:i:s"), 1);
...
function createMessageTemplate($arrayJson, $row)
{
  ...
  $arrayPostData['messages'][0]['text'] = "คุณ {$driver_name} / ทะเบียนรถ {$car_license_number} อีก 30 นาทีเตรียมตัวเข้าช่องโหลด";
